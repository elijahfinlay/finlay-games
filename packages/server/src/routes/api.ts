...
apiRouter.get('/leaderboard', async (_req, res) => {
  try {
    ...
    console.error('[API] Failed to load leaderboard:', err);
    res.json({ players: [], dbConnected: false });
  }
});
...
apiRouter.get('/matches/recent', async (_req, res) => {
  try {
    ...
    console.error('[API] Failed to load recent matches:', err);
    res.json({ matches: [], dbConnected: false });
  }
});
...
apiRouter.get('/players/:id/stats', async (req, res) => {
  try {
    ...
    console.error('[API] Failed to load player stats:', err);
